...
- **Hội thoại kịch bản → SSML:** dạng `NHÂN VẬT *(sắc thái)* Lời thoại` (và biến thể không có sắc thái, hoặc chỉ có chỉ dẫn đứng đầu) tách thành **hai segment**: người dẫn đọc tên (+ vai/chú thích) bằng giọng narrator, rồi giọng nhân vật đọc lời thoại với `<break>` / `<prosody>` theo sắc thái (`_dialogue_prosody`: trầm/khàn, thì thầm, hét, run sợ, tức giận…). Đoạn tiếp theo vẫn thuộc cùng nhân vật nếu mở bằng gạch ngang (`-` / `–` / `—` / `―` / …) **hoặc** là cả đoạn in nghiêng không gạch (vd `*Điều thứ nhất: …*`). Prose thường (vd `AFONSO (58 tuổi) đứng trên boong`) **không** bị nhận nhầm thành thoại. Admin có thể tắt đọc cue qua `speak_speaker_names` / `speak_stage_directions`.
...
**Mark hiện tại:** sách mở tinh gọn + **2 sóng nghe** (nét mỏng hơn bản cũ). **Wordmark = chỉ chữ Fraunces "Read"** — không kèm icon cạnh chữ ở header; mark dùng riêng cho favicon, login, app icon.
...
| File | Vai trò |
|------|---------|
| `scripts/brand/logo.mjs` | **Nguồn sự thật** — mark sách mở + 2 sóng nghe |
| `scripts/brand/build.mjs` | Wordmark typography-only + xuất SVG/PNG |
| `scripts/brand/read-letters.path` | Outline chữ “Read” (Fraunces gốc) |
...
**Quy ước UI**
- **Wordmark** = chữ Fraunces **một mình** → header / reader chrome (không icon kèm chữ)
- **Mark** = sách mở + 2 sóng nghe → login, favicon, app icon

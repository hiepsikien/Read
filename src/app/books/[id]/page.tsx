...
import { getBook, getChapters, hasPurchase } from "@/lib/db";
...
export default async function BookDetailPage({ params }: Props) {
  ...
  const isOwner = session.user?.id === book.publisher_id;

  if (book.status !== "published" && !isOwner) notFound();

  const chapters = getChapters(id);
  const owned =
    book.price_cents === 0 ||
    isOwner ||
    (session.user ? hasPurchase(session.user.id, book.id) : false);
    ...
    <div className="fade-up mx-auto max-w-3xl">
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        ...
        {!owned && book.price_cents > 0 && (
          <PurchaseButton bookId={book.id} priceCents={book.price_cents} signedIn={Boolean(session.user)} />
        )}
        {isOwner && (
          <Link

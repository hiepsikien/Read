...
export default function ManageBookPage() {
  ...
  async function load() {
    ...
    if (!response.ok) {
      setError(payload.error || "Could not load book.");
      return;
    }
    setData(payload);
  ...
  async function splitChapters() {
    ...
    setMessage("");
    setError("");
    const response = await fetch(`/api/books/${params.id}/split`, { method: "POST" });
    const payload = await response.json();
    setBusy("");
    if (!response.ok) {
      setError(payload.error || "Split failed.");
      return;
    }
  ...
  async function publish() {
    ...
    setMessage("");
    setError("");
    const response = await fetch(`/api/books/${params.id}/publish`, { method: "POST" });
    const payload = await response.json();
    setBusy("");
    if (!response.ok) {
